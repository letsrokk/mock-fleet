...
- [ ] Modify the catalog between the updater's read and update. The stale update fails with a Kubernetes `resourceVersion` conflict, does not overwrite the concurrent change, and does not retry in the same Job.
- [ ] An updater reconciliation does not replace either live WireMock Pod. Existing mocks keep their runtime versions until an explicit lifecycle action applies a saved desired version.
