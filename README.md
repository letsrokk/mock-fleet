...
The MCP service uses stateful Streamable HTTP and therefore runs one replica. The chart manages a WireMock version catalog with one image policy. When Mock Ops is disabled, `wiremock.supportedImageTags` is the exact allowlist and must include the exact tag from `wiremock.containerImage`. When enabled, `mockOps.allowedVersionRange` (default `[3.0,4.0)`) controls all selectable versions, and supported tags provide initial seeds within that interval. Connected Helm upgrades preserve allowed runtime defaults and discovered choices, apply changed policy, and use `wiremock.containerImage` when the current default is no longer allowed. Existing excluded pins stay retained and runnable; new pins and defaults must satisfy the policy. Active pods do not restart. Helm 4 upgrades require `--server-side=false` for the catalog shared with Fleet Mock Ops; server-side apply can conflict on controller-managed fields.

Saved UI overrides live in a runtime ConfigMap outside the chart’s desired resources. A Helm install/upgrade or Argo CD full sync initializes it only when missing and protects existing data from overwrite and pruning. See the [initialization and Argo CD migration requirements](deploy/helm/mock-fleet/README.md#saved-user-configuration).
...
The optional Fleet Mock Ops CronJob is disabled by default. Enable it with `mockOps.enabled=true`. It reads Registry V2 tags, filters selectable versions and advances the catalog default within the configured numeric interval, keeps the latest configured minor lines plus any constrained default selectable, and retains exact versions still referenced by baseline or user configuration. It updates one ConfigMap with Kubernetes `resourceVersion` concurrency and never restarts active mocks. See the [Fleet Mock Ops chart values and reconciliation contract](deploy/helm/mock-fleet/README.md#fleet-mock-ops).

Private registries use existing Secrets: `wiremock.serviceAccount.imagePullSecrets` attaches Kubernetes pull-secret references to the managed WireMock ServiceAccount; `mockOps.registry.credentialsSecretName` supplies separate `username`/`password` discovery credentials. Configure pull-secret references yourself when using an external ServiceAccount. See the [private-registry example](deploy/helm/mock-fleet/README.md#fleet-mock-ops). The removed `mockOps.defaultVersionConstraint` key is rejected; use an interval such as `[3.13,3.14)` instead.
