...
local-deploy:
	...
	@bin/local/deploy.sh --namespace $(call shell-quote,$(NAMESPACE))$(if $(filter true,$(TINYPROXY)), --tinyproxy, --no-tinyproxy)$(if $(filter true,$(LOGS)), --logs)$(if $(filter true,$(DEV)), --remote-dev api,$(if $(filter api proxy,$(DEV)), --remote-dev $(DEV)))$(if $(filter true,$(PORT_FORWARD)), --port-forward)$(if $(filter-out false,$(REBUILD)), --rebuild $(call shell-quote,$(REBUILD)))
