...
help:
	...
	@echo "  make local-deploy [NAMESPACE=name] [LOGS=true] [DEV=true|api|proxy] [PORT_FORWARD=true] [TINYPROXY=false] [REBUILD=dash|api|proxy|mcp|mock-ops|all]"
	@echo "  make local-destroy [NAMESPACE=name] [RELEASE=name] [DELETE_NAMESPACE=true]"

local-deploy:
	$(if $(call is-one-of,$(TINYPROXY),true false),,$(error TINYPROXY must be true or false))
	...
	@bin/local/deploy.sh --namespace $(call shell-quote,$(NAMESPACE))$(if $(filter false,$(TINYPROXY)), --no-tinyproxy)$(if $(filter true,$(LOGS)), --logs)$(if $(filter true,$(DEV)), --remote-dev api,$(if $(filter api proxy,$(DEV)), --remote-dev $(DEV)))$(if $(filter true,$(PORT_FORWARD)), --port-forward)$(if $(filter-out false,$(REBUILD)), --rebuild $(call shell-quote,$(REBUILD)))
