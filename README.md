...
`make local-deploy` neither checks nor changes CNI capability. The bridge CNI in the Minikube cluster reviewed for this release accepted the policies but did not enforce them. Do not treat that profile as providing the private-network egress boundary. Run the [chart's internal-connectivity denial probe](deploy/helm/mock-fleet/README.md#verify-networkpolicy-enforcement) before using a cluster for untrusted mocks.
