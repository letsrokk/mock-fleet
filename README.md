...
The full Minikube/SeaweedFS contract suite is opt-in because it creates a namespace, cluster-scoped PV, WireMock pods, and S3 data. Run its static self-test anywhere with `bin/cluster-e2e.sh --self-test`. For a live run, pre-create an isolated SeaweedFS bucket and provide its name plus CSI settings as described by `bin/cluster-e2e.sh --help`. The script uses a unique run ID, cleans up through a trap, and is safe to rerun. GitHub Actions exposes the same suite only through the manual `Cluster E2E` workflow; it is not a push gate.
