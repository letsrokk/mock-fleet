...
help:
	...
	@echo "  make local-deploy [NAMESPACE=name] [LOGS=true] [DEV=true|api|proxy] [PORT_FORWARD=true]"
...
local-deploy:
	...
	@bin/local/deploy.sh --namespace $(call shell-quote,$(NAMESPACE))$(if $(filter true,$(LOGS)), --logs)$(if $(filter true,$(DEV)), --remote-dev api,$(if $(filter api proxy,$(DEV)), --remote-dev $(DEV)))$(if $(filter true,$(PORT_FORWARD)), --port-forward)
